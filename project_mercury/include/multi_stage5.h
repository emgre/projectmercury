...
{
public:
	virtual bool LoadPanel (VESSEL *vessel, int id) {return true;};
	virtual bool PanelRedrawEvent (VESSEL *vessel, int id, int event, SURFHANDLE surf){return true;};
	virtual bool PanelMouseEvent (VESSEL *vessel, int id, int event, int mx, int my){return true;};
...
public:
	...
	virtual void SetClassCaps(FILEHANDLE cfg);
	virtual void LoadState (FILEHANDLE scn, void *vs);
};
...
void IStage::SetClassCaps(FILEHANDLE cfg)
{
};

void IStage::LoadState (FILEHANDLE scn, void *vs){
}
...
public:
	...
	int clbkConsumeKey (const char *keystate);
...
//	bool clbkVCMouseEvent (int id, int event);
